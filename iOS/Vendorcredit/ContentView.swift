...
struct ContentView: View {
    ...
    @State private var editingEntry: VendorcreditEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Track small personal loans between friends until they're paid back.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: VendorcreditEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (VendorcreditEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: VendorcreditEntry? = nil, onSave: @escaping (VendorcreditEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = VendorcreditEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
